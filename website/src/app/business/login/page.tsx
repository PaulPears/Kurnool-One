...
export default function BusinessLoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isSignUp) {
        if (!name.trim()) {
          throw new Error('Please enter your Business Owner Name');
        }
      ...
      <div className="max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-12 gap-8 items-center">
        ...
        <div className="md:col-span-5 space-y-6">
          ...
          <p className="text-slate-600 text-sm leading-relaxed">
            Welcome to the official Kurnool One Merchant Portal. Connect with thousands of local customers, receive direct phone & WhatsApp inquiries, and manage your live storefront.
          </p>
          ...
          <div className="space-y-3 pt-2">
            {[
              ...
              { icon: Percent, text: 'Publish discount deals & coupons to Kurnool Deals' },
              { icon: TrendingUp, text: 'Live analytics of page views and customer engagement' },
            ].map((item, idx) => (
        ...
        <div className="md:col-span-7 bg-white rounded-3xl shadow-xl border border-blue-100/80 p-6 sm:p-8">
          ...
          <div className="mb-6 p-4 rounded-2xl bg-amber-50/80 border border-amber-200">
            ...
            <p className="text-xs text-amber-800 mb-3 leading-relaxed">
              Login immediately with pre-configured merchant credentials (<code className="font-mono bg-amber-100 px-1 py-0.5 rounded text-amber-900">merchant@kurnoolone.com</code>):
            </p>
            <button
              onClick={handleDemoMerchant}
              disabled={demoLoading}
              className="w-full flex items-center justify-center gap-2 py-2.5 px-4 rounded-xl bg-amber-600 hover:bg-amber-700 text-white font-bold text-xs shadow-sm transition-all active:scale-[0.99] disabled:opacity-50"
            >
              <Building2 className="w-4 h-4" />
              {demoLoading ? 'Logging In...' : 'Launch Business Dashboard as Demo Merchant'}
            </button>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-3 px-4 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-bold text-sm shadow-md shadow-blue-600/20 transition-all active:scale-[0.99] disabled:opacity-50 flex items-center justify-center gap-2 mt-2"
            >
              {loading ? 'Authenticating...' : isSignUp ? 'Create Merchant Account' : 'Sign In to Business Portal'}
          ...
          >
            <span>Skip Login (Explore Business Dashboard as Guest)</span>
            <ArrowRight className="w-3.5 h-3.5" />
          </button>

          <div className="mt-6 pt-4 border-t border-slate-100 flex items-center justify-between text-xs text-slate-500">
            <Link href="/register-business" className="hover:text-blue-600 font-bold">
